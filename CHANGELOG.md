...
- `--short` flag on `videos`, `youtube-meta`, and `youtube-upload` — a "short" variant that caps **static** slides (Instagram exports photo-with-music stories as 1 fps MP4s) at `--max-slide-duration` seconds while leaving genuine video at full length. Detection uses frame rate as a cheap pre-filter and then a definitive frame-identity check (frames sampled at start/middle/end, decoded at 64×64 and hashed), because frame rate alone misclassifies ~15% of 1 fps clips that are real video.
- `--all-variants` flag on `videos`, `youtube-meta`, and `youtube-upload` — produces all four outputs in one run: portrait/landscape × full/short.
- `--max-slide-duration SECONDS` on `videos` (default 10) — the cap applied to static slides in the short variant.
- Short variants get their own output dirs (`videos_short/`, `videos_landscape_short/`), metadata dirs (`youtube_short/`, `youtube_landscape_short/`), a `· Short` title suffix, and their own YouTube playlists (`… · Short`, `… · 16:9 · Short`).
- `--both-formats` flag on `videos`, `youtube-meta`, and `youtube-upload` — processes both portrait and landscape in a single run (encodes/generates/uploads each highlight once per format, portrait then landscape)
